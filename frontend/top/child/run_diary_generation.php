...
try {
    ...
    if (!is_dir($getPlaceDir) || !is_file($getPlaceDir . DIRECTORY_SEPARATOR . $getPlaceFile)) {
        throw new RuntimeException('get_place.php が見つかりません: ' . $getPlaceDir);
    }
    if (!is_dir($generateDiaryDir) || !is_file($generateDiaryDir . DIRECTORY_SEPARATOR . $generateDiaryFile)) {
        throw new RuntimeException('run_all.php が見つかりません: ' . $generateDiaryDir);
    }

    $command1 = 'cd /d "' . $getPlaceDir . '" && "' . $phpExecutablePath . '" "' . $getPlaceFile . '" 2>&1';
    runDiaryCommand('get_place.php', $command1, $debugId);

    $command2 = 'cd /d "' . $generateDiaryDir . '" && "' . $phpExecutablePath . '" "' . $generateDiaryFile . '" 2>&1';
